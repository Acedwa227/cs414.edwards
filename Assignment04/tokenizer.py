...
KEYWORDS = {"ls", "cd", "cat", "print", "exec", "set", "echo"}
...
def tokenize(code: str) -> List[Token]:
    ...
    for mo in re.finditer(master_regex, code):
        ...
        if kind == "WORD":
            ...
            tokens.append((TokenType.VARIABLE, value))

        elif kind == "EQ":
            tokens.append((TokenType.EQ, value))

        elif kind == "OPERATOR":
            tokens.append((TokenType.OPERATOR, value))

        elif kind == "LPAREN":
            tokens.append((TokenType.LPAREN, value))

        elif kind == "RPAREN":
            tokens.append((TokenType.RPAREN, value))
